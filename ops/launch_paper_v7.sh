#!/bin/bash
# EvoSteer Qwen3.5-9B curve run v7 (paper benchmarks, full method with skill admission, fresh start).
...
OUT=/workspace/evosteer_runs/9b_paper_v7_${STAMP}
...
echo "$OUT" > /workspace/evosteer_runs/active_9b_paper_v7.out
exec /workspace/evosteer/venv313/bin/python -u -m skillev.evosteer_cli train \
  --config /workspace/evosteer_9b_paper_v7_config.json \
