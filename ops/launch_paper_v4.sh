#!/bin/bash
# EvoSteer 27B curve run v4 (paper benchmarks, full method with skill admission): fixed FP8 loading, FP8 input gradients, activation
...
OUT=/workspace/evosteer_runs/27b_paper_v4_${STAMP}
...
echo "$OUT" > /workspace/evosteer_runs/active_27b_paper_v4.out
exec /workspace/evosteer/venv313/bin/python -u -m skillev.evosteer_cli train \
  --config /workspace/evosteer_27b_paper_v4_config.json \
