...
def _f1(prediction: str, answer: str) -> float:
    p, g = _hotpot_norm(prediction).split(), _hotpot_norm(answer).split()
    if not p or not g:
        return float(p == g)
...
def _math_prediction(output: str) -> Decimal | None:
    ...
    Priority: last ``####`` line, last ``\\boxed{}``, the first number after the
...
def _hotpot_score(output: str, answer: str) -> float:
    ...
    return max(_f1(_short_answer(output), answer), _f1(output, answer))
...
def _code_score(output: str, row: dict[str, Any]) -> float:
    ...
    if not isinstance(setup, str) or not tests:
        return 0.0
    # Assertions stay verifier-side and are passed through a child process. A
    # per-run sentinel printed after the last assertion rejects programs that
    # exit early with status 0 instead of passing the tests.
    ...
    with tempfile.TemporaryDirectory(prefix="evosteer-mbpp-") as temp:
        ...
        result = subprocess.run(
            ...
            timeout=8,
            check=False,
            preexec_fn=_child_limits if sys.platform.startswith("linux") else None,
        )
    passed = result.stdout.decode("utf-8", "replace").splitlines()[-1:] == [sentinel]
    return float(result.returncode == 0 and passed)
...
def task_factory(*, policy: Any, config: Any) -> tuple[TaskBinding, ...]:
    ...
    for row in rows:
        ...
        task = EvoTask(task_id, kind, prompt, reset_id="initial", environment_config_id=f"curve-{kind}@1")
        ...
        def session(request: SessionRequest, *, _row=row, _kind=kind, _task=task) -> TaskSession:
            ...
            return TaskSession(
                ...
                evaluate,
                reset_receipt=ResetReceipt(_task.identity, _task.reset_id, _task.environment_config_id, _task.identity, f"{_task.task_id}:{request.seed}", request.seed),
