#!/bin/bash
# EvoSteer Qwen3.5-9B curve run v12b: v7 + skill warmup (no skills in batches 1-8; author writes one skill per family at batch 9 from a pooled evidence buffer) + sync-free update.
...
OUT=/workspace/evosteer_runs/9b_paper_v12b_${STAMP}
...
echo "$OUT" > /workspace/evosteer_runs/active_9b_paper_v12b.out
exec /workspace/evosteer/venv313/bin/python -u -m skillev.evosteer_cli train \
  --config /workspace/evosteer_9b_paper_v12b_config.json \
